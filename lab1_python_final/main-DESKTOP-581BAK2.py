...
def plot_model(subplot, x_train, y_train, x, y_obj, y_model, x_val=None, y_val=None, train_err=None, val_err=None):
    ...
    sub.plot(x_train, y_train, 'o', markerfacecolor='none',
    ...
    if x_val is not None and y_val is not None:
        sub.plot(x_val, y_val, 'o', markerfacecolor='none',
...
if __name__ == "__main__":
    ...
    for i in range(8):
        ...
        plot_model(sub, data['x_train_8'],
                   data['y_train_8'], x_plot, y_obj, y_model)
    ...
    for i in range(8):
        ...
        y_model = polynomial(x_plot, w)
        sub = fig.add_subplot(2, 4, i + 1)
        plot_model(sub, data['x_train_50'],
                   data['y_train_50'], x_plot, y_obj, y_model)
        sub.set_title("M = {}".format(i))
    ...
    M_values = range(0, 7)
    w, train_err, val_err = model_selection(
        data['x_train_50'], data['y_train_50'], data['x_val_20'], data['y_val_20'], M_values)
    M = np.shape(w)[0]-1
    y_model = polynomial(x_plot, w)
    ...
    plot_model(sub, data['x_train_50'], data['y_train_50'], x_plot, y_obj,
               y_model, data['x_val_20'], data['y_val_20'], train_err, val_err)
